...
namespace JwtAPI.Controllers
{
    ...
    public class AuthorizationController : ControllerBase
    {
        ...
        public async Task<IActionResult> Registration([FromBody] RegistrationModel model)
        {
            ...
            var userExists = await userManager.FindByNameAsync(model.Usename);
            if (userExists != null)
            {
                status.StatusCode = 0;
                status.Message = "Invalid username";
        ...
        //after registering admin, this code will comment, because just need one admin
        [HttpPost]
        public async Task<IActionResult> RegistrationAdmin([FromBody] RegistrationModel model)
        {
            ...
            if (!result.Succeeded)
            {
                ...
                foreach (var error in result.Errors)
                {
                    ...
                    // Example: logger.LogError($"Error: {error.Description}");
                }
                return Ok(status);
            }
